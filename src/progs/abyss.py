...
class abyss(EXTprogram):
    """Trinity program assembles sequencing reads into contigs for genomic data"""
    ...
    def assemble(self, inpath, forward, reverse, unpaired_in, outdir, kmers, coverage, npar, mpirun, cores, parallel):
        ...
        executor.setconfig("abyss-pe")

        # TESTING INPUT PARAMETER VALUES

        mpi, kmers, coverage = abyss_pars(kmers, coverage, npar, mpirun, cores, parallel)

        if abyss.debug:
            pdb.set_trace()

        # SET DRY RUN AND DEBUGGING MODES FOR SUBCLASSES
        abyss.run_dry(getinput, output, makenewdir, executor)
        abyss.set_debug(executor)
        ...
        # MAKE LOG DIR
        log_dir = makenewdir(name="abyss_logs", fullname="LOG")
        logging.info("Find all ABySS log files in '{0:s}'".format(os.path.basename(log_dir.path)))
        ...
        TASKS = abyss_tasks(paired, unpaired, kmers, coverage, npar, mpi, cores, log_dir.path, tmpdir.path)
        ...
        if mpirun:
            jobs = []
            for task in TASKS:
                result = abyss_run(task[0], task[1], task[2], task[3], task[4], task[5])
                jobs.append(result)
        else:
            processes_requested = set_threads("ABySS", len(TASKS), abyss.threads // cores, userdef=False)
        ...
        # SAVE STATS
        if abyss.stats and not abyss.dryrun:
            abyss_stats(jobs)

        # DELETE TEMPORARY DIRECTORY
        if not abyss.keeptmp:
            tmpdir.delete()


def abyss_run(kmeroutpath, logpath, infiles, pars, mpi, cores):
    """Runs one instance of ABySS analysis"""
    ...
    run_executor(executor(
        program="abyss-pe",
        params=[kmeroutpath, mpi, cores, pars, kmername, infiles],
        conditions={"positive": [cores], "pathexists": [kmeroutpath]},
        custom_arg_string=abyss.extra + " > " + kmeroutlog + " 2>&1"
    ))
    if abyss.dryrun:
        return "DRYRUN"
    else:
        if len(os.listdir(kmeroutpath)) > 0:
            ...
            assembly_file = os.path.join(output.path, kmername + abyss.suffix + '.fa')
            shutil.copy2(outfile, assembly_file)

            shutil.rmtree(kmeroutpath)
            logging.debug("Finished assembling for '{0:s}' library. All resulted contigs were saved to {1:s}".format(lib, kmername + abyss.suffix + ".fa"))
            return assembly_file
        else:
            logging.error("No files were found in the ABySS output folder")
            raise EXONtoolsError("ABySS output error")


def abyss_pars(kmers, coverage, npar, mpirun, cores, parallel):
    """Checking abyss input parameters"""

    if abyss.extra:
        logging.warning("The following arguments will be added to 'ABySS' command line:")
        logging.warning(abyss.extra)

    logging.debug("Testing ABySS input settings")
    if kmers:
        if kmers == "program default":
            ...
            try:
                ...
                raise EXONtoolsError("Kmer format error")
            for k in kmers:
                if k < 10:
                    logging.error("All k-mer must be integers greater or equal 10 bp")
                    raise EXONtoolsError("Wrong format of k-mer length")
            kmers.sort()
            logging.info("ABySS analysis will use the following k-mers: {0}".format(kmers))
    ...
    if coverage == 'program default':
        coverage = None
        logging.info("All contigs with the coverage lower than the squared median will be dismissed")
        ...
        for c in coverage:
            if c < 5:
                ...
                raise EXONtoolsError("Wrong settings for coverage option")
        logging.info("ABySS analysis will use the following coverage thresholds: {0}".format(coverage))
    ...
    if mpirun:
        ...
        if stdoutdata and os.path.exists(stdoutdata.strip()):
            ...
            mpi = " mpirun=" + mpirun + " np=" + str(parallel)
    ...
    logging.debug("ABySS program settings: OK")
    return mpi, kmers, coverage


def abyss_tasks(paired, unpaired, kmers, coverage, npar, mpi, cores, logpath, tmppath):
    """Prepare TASK list for ABySS analysis"""

    logging.debug("Preparing ABySS input command arguments for parallel analysis")
    ...
    for lib in librs:
        ...
        if lib + "_paired" in paired:
            infiles = "in='{0:s}' se='{0:s}".format(" ".join(paired[lib + "_paired"]))
            if lib + "_unpaired" in unpaired:
                infiles = "{0:s} {1:s}".format(infiles, unpaired[lib + "_unpaired"][0])
            infiles = infiles + "'"
        else:
            if lib + "_unpaired" in unpaired:
                infiles = "se={0:s}".format(unpaired[lib + "_unpaired"][0])
            else:
                logging.error("Input files for '{0:s}' library are not provided".format(lib))
                raise EXONtoolsError("No input files are provided for assembly analysis")
        ...
        if kmers and coverage:
            for k in kmers:
                for c in coverage:
                    ...
                    makenewdir(kmeroutpath)
                    pars = "k={0:d} c={1:0.1f} e={2:d} n={3:d}".format(k, float(c), c, npar)
                    if mpi:
                        TASKS.append([kmeroutpath, logpath, infiles, pars, mpi, cores])
                    else:
                        TASKS.append(worker(abyss_run, [kmeroutpath, logpath, infiles, pars, mpi, cores]))
        elif kmers and not coverage:
            for k in kmers:
                kmername = "{0:s}_km{1:d}".format(lib, k)
                kmeroutpath = os.path.join(libouttmp, kmername)
                makenewdir(kmeroutpath)
                pars = "k={0:d} n={1:d}".format(k, npar)
                if mpi:
                    TASKS.append([kmeroutpath, logpath, infiles, pars, mpi, cores])
                else:
                    TASKS.append(worker(abyss_run, [kmeroutpath, logpath, infiles, pars, mpi, cores]))
        ...
        logging.debug("ABySS input command arguments are created: OK")
    return TASKS


def abyss_stats(paths):
    """Write stats for ABySS analysis"""

    logging.debug("Collecting statistical data on assemblies")

    statpath = os.path.join(output.path, "STATS_abyss_assembly.csv")
    ...
    with open(statpath, 'w') as statout:
        ...
        for path in sorted(paths, key=natural_sort):
            x = metrics(path)
            csv_writer.writerow([x.ID.replace(abyss.suffix, ""), x.totnum, x.long500, x.long1000, x.long5000, x.long10000, x.longest, x.min, x.N75, x.N50, x.N25, x.max, x.L50, x.L75, x.NG, x.mean, x.esize, x.totlen, x.gc, x.nbases, x.n100])
